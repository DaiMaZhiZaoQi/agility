...
public class SysPermissionServiceImpl implements SysPermissionService {
    ...
    @Autowired
    private SysUserPermissionMapper mSysUserPermissionMapper;
...
//        PageHelper.startPage(page, rows);
	...
	@Override
	public boolean updatePermission(String permiGroupName) {
		System.out.println("updatePermission-->"+permiGroupName);
		SysPermissionGroup spg = sysPermissionGroupMapper.selectByGroupName(permiGroupName);
//		System.out.println("updatePermission-->"+spg.toString());
		if(spg==null)return false;
		Long spgId = spg.getId();
		Long updateStatus = sysPermissionGroupMapper.updateStatus(spgId,2);
		if(updateStatus>0) {
			sysPermissionMapper.updateStatus(2,spgId);
			List<SysPermission> listSysPermission = sysPermissionMapper.selectByPgId(spgId);
			// 测试是否返回集合类id
			...
			Long result = mSysUserPermissionMapper.updateByPermissionId(listId,2);
