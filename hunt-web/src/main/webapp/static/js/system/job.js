job_tool = {
    ...
    checkValidatebox: function () {
        if (!$('#job_dialog input[id="job_name"]').validatebox('isValid')) {
            common_tool.messager_show('请输入名称');
            return false;
        } else if (!$('#job_dialog input[id="job_fullName"]').validatebox('isValid')) {
            common_tool.messager_show('请输入全称');
            return false;
        } else if (!$('#job_dialog input[id="job_description"]').validatebox('isValid')) {
    ...
    save: function () {
        ...
        var name = $('#job_dialog input[id="job_name"]').val();
        var fullName = $('#job_dialog input[id="job_fullName"]').val();
        var description = $('#job_dialog input[id="job_description"]').val();
    ...
    update: function () {
        ...
        var id = $('#job_dialog input[id="job_id"]').val();
        var name = $('#job_dialog input[id="job_name"]').val();
        var fullName = $('#job_dialog input[id="job_fullName"]').val();
        var description = $('#job_dialog input[id="job_description"]').val();
        var parent_job = $('#job_dialog table[id="job_dialog_parent_tree"]').treegrid("getChecked")[0];
    ...
    init_main_view: function () {
        $("#job_grid").treegrid({
            ...
            columns: [[
                ...
                {title: "组织名称", field: "sysOrganizationName", width: 250},
                {title: "角色名称", field: "sysRoleName", width: 250},
                {title: "说明", field: "description", width: 320},
                {
